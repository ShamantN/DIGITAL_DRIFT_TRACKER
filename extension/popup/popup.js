...
document.addEventListener('DOMContentLoaded', () => {
  ...
  chrome.storage.local.get(['auth_token', 'user_id', 'user_email'], (data) => {
    if (data.auth_token && data.user_id) {
      ...
      list.innerHTML = '<li class="loading">Please login to see your stats.</li>';
    }
  });
  ...
  if (link) {
    link.addEventListener('click', (e) => {
      ...
      chrome.tabs.create({ 
        url: `${dashboardUrl}?token=${authToken}` 
      });
    });
  }
  ...
  function handleLogout() {
    // Clear storage
    chrome.storage.local.remove(['auth_token', 'user_id', 'user_email'], () => {
  ...
  function fetchAnalytics(token) {
    // We need the user ID, which we get from the auth token on the backend
    fetch(`http://127.0.0.1:8000/api/dashboard/analytics?period_days=1`, {
      headers: {
